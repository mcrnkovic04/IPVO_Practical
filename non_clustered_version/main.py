...
def cleanup():
    ...
    cursor.execute("DROP TABLE IF EXISTS non_clustered_table;")
    conn.commit()
...
cursor.execute("CREATE TABLE non_clustered_table (id SERIAL PRIMARY KEY, name VARCHAR);")
conn.commit()
...
print(f"Total time for OLTP workload with non-clustered tables (including cache): {total_time} seconds")
